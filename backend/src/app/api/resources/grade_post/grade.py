...
async def grade_post(post_id: int, data: GradePostSchema, user: UserSchema = Depends(UserService.get_current_user),
...
@router.delete('', summary="Delete Grade post")
async def delete_grade_post(post_id: int, user: UserSchema = Depends(UserService.get_current_user),
                            db: Session = Depends(get_session)):
    post: PostModel = PostService(db, user).get(post_id)
    if post is None:
        raise HTTPException(
    ...
    if not GradeService(db, user).delete(post=post):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"User not like/dislike post_id={post_id}",
        )
